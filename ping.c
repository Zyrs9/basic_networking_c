...
size_t build_icmp_echo(uint16_t id, uint16_t seq,
    ...
    hdr.id   = htons(id); hdr.seq = htons(seq);
...
// resolve host -> IPv4 (use inet_pton first)
...
// send once, wait for our reply (id/seq). returns 0 ok, -1 timeout/err.
// out: rtt_ms, reply ip string, reply ttl
static int send_recv_once(int sockfd,
...
{
    uint8_t packet[1500];
    size_t pkt_len = build_icmp_echo(id, seq, payload, pay_len, packet, sizeof(packet));
    ...
    for (;;) {
        ...
        if (icmp_hdr->type == ICMP_ECHOREPLY &&
            ...
            if (out_ip && out_ip_sz > 0)
                inet_ntop(AF_INET, &ip_hdr->saddr, out_ip, out_ip_sz);
            if (out_ttl) *out_ttl = ip_hdr->ttl;
            return 0;
        }

        // common ICMP errs: print and stop
        ...
        // else: not our reply -> keep waiting
    }
}

// parse int like "-c 5". returns 0 on ok.
...
static void usage(const char *argv0) {
    fprintf(stderr,
        "usage: %s [-c count] [-i ms] [-W ms] [-t ttl] host\n"
        "  -c N   count (default 4)\n"
        "  -i MS  interval millis (default 1000)\n"
        "  -W MS  recv timeout millis (default 5000)\n"
        "  -t N   TTL/hops (optional)\n", argv0);
}
...
int main(int argc, char **argv) {
    ...
    // getopt
    int opt;
    while ((opt = getopt(argc, argv, "c:i:W:t:")) != -1) {
        switch (opt) {
    ...
    // open raw socket (need sudo/root)
    ...
    // show the first packet bytes (debug)
    {
        uint8_t pkt[128];
    ...
    for (int i = 0; i < count; ++i) {
        ...
        int ttl = -1;
        int ok = send_recv_once(sockfd, &dst, id, seq,
                                payload, pay_len, timeout_ms,
